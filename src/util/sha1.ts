...
const workers = (() => {
    ...
    } catch (e) { console.log('init worker error', e) }
...
async function hash(pieceIndex: number, pieceData: Uint8Array, callback: HashCallback) {
    return new Promise<void>((resolve, reject) => {
        if (workers.length > 0) {
            doHashWithWorker(pieceIndex, pieceData, callback, resolve)
        } else {// 非 worker 模式
...
// 等待队列
const waitingTask: { req: Req, callback: HashCallback, resolve: () => void }[] = []

function doHashWithWorker(pieceIndex: number, pieceData: Uint8Array, callback: HashCallback, resolve: () => void) {
    ...
    if (selectedWorker != null) { // 发送消息给 worker 处理
        resolve()// 有 worker 可以处理，就可以继续读取接下来的数据
        ...
        selectedWorker.onmessage = (e) => {
            ...
            if (task) {
                ...
                doHashWithWorker(task.req.pieceIndex, task.req.pieceData, task.callback, task.resolve)
            }
        }
    } else { // 等待有可用的 worker
        console.log('waiting a worker', req)
        waitingTask.push({ req, callback, resolve })
    }
}
